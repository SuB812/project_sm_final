...
# ── 전역 CSS (pseudo-element / fixed 없이) ──────────────
st.markdown("""<style>
...
S = {
    ...
    "hero_title_light": "font-weight:300;color:rgba(183,220,255,0.8);",
    "hero_divider": (
    ...
    "badge_blue": (
        ...
        "background:linear-gradient(135deg,#E6F1FB,#d0e8ff);color:#0C447C;"
        "display:inline-block;margin-bottom:6px;"
    ),
    "bubble_user": (
        "background:linear-gradient(135deg,#0C447C,#185FA5);color:#fff;"
        "border-radius:16px 16px 4px 16px;padding:10px 14px;"
        "margin:6px 0 6px 15%;font-size:14px;font-weight:400;line-height:1.6;"
        "font-family:'Pretendard',sans-serif;"
    ),
    "bubble_bot": (
        "background:#f0f4ff;border:0.5px solid rgba(24,95,165,0.12);color:#1a2340;"
        "border-radius:16px 16px 16px 4px;padding:10px 14px;"
        "margin:6px 15% 6px 0;font-size:14px;font-weight:300;line-height:1.6;"
        "font-family:'Pretendard',sans-serif;"
    ),
    "hist_header": (
    ...
    "hist_item": (
        ...
        "border:0.5px solid rgba(24,95,165,0.1);border-radius:10px;"
        "margin-bottom:7px;background:#f8faff;"
    ),
}
...
# ──────────────────────────────────────────────────────
def get_law_chunks(law_graph, industry, query, max_chunks=20):
    all_chunks = []
...
# ──────────────────────────────────────────────────────
def get_similar_cases(kg, industry, query, max_cases=3):
    results = []
    keywords = [w for w in (industry + " " + query).split() if len(w) > 1]
...
# QnA 컨텍스트 빌더
# ──────────────────────────────────────────────────────
def build_qna_context(law_graph, kg, question):
    parts = []
    try:
...
def analyze_ad(law_graph, kg, industry, status, suspended, ad_copy):
    client = get_client()
...
아래 광고 문구의 법적 리스크를 진단하고 반드시 유효한 JSON 형식으로만 응답하세요. 다른 텍스트는 절대 포함하지 마세요.
...
def answer_qna(law_graph, kg, question, history, prior_context=None):
    client  = get_client()
    context = build_qna_context(law_graph, kg, question)

    system = f"""당신은 대한민국 표시광고법 전문 AI 어시스턴트입니다.
아래 법령과 의결서 사례를 참고하여 질문에 답변하세요.

규칙:
1. 법령/의결서에 근거가 있을 때만 구체적으로 답변합니다.
2. 근거가 불충분하거나 사안이 복잡하면 "법률 전문가 상담을 권장합니다"라고 명시합니다.
3. 유사 의결서 사례가 있으면 사건명/의결번호와 함께 언급합니다.
...
def bottom_nav():
    ...
    for col, (label, page) in zip(cols, NAV):
        with col:
            btn_label = f"**{label}**" if active == page else label
...
def page_home():
    st.markdown(
        ...
        f'<h1 style="{S["hero_title"]}">Ad<span style="{S["hero_title_light"]}">Doctor</span></h1>'
        f'<div style="{S["hero_divider"]}"></div>'
        f'</div>',
        unsafe_allow_html=True,
    )
...
def page_result():
    ...
    # 리스크 헤더
    st.markdown(
        f'<div style="{S["risk_header"]}">'
        f'<div style="{S["risk_badge"]}">{dot(level, 8)}{level}</div>'
        f'<p style="{S["risk_summary"]}">{summary}</p>'
        f'</div>',
        unsafe_allow_html=True,
    )
...
def page_qna(law_graph, kg):
    ...
    question = st.text_area(
        "질문", placeholder="예: 건강기능식품에서 '임상 검증'이라는 표현을 쓸 수 있나요?",
...
def page_history():
    ...
    for row in history:
        ...
        dot_color = RISK_COLOR.get(level,"#aaa")
        st.markdown(
            f'<div style="{S["hist_item"]}">'
            f'<span style="width:8px;height:8px;border-radius:50%;background:{dot_color};flex-shrink:0;display:inline-block;"></span>'
